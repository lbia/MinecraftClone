...
int getYMassima(World *world, int x, int z){
    return world->occupati[getIndiceMondo(x, z)][getOccupati(x, z)][world->occupati[getIndiceMondo(x, z)][getOccupati(x, z)][0]];
}
...
int getIndiceMondoDaCoordinateMondo(int mondoX, int mondoZ){
    return (mondoX + HALF_MONDI_MASSIMI) * MONDI_MASSIMI + mondoZ + HALF_MONDI_MASSIMI;
}
...
int getCube(int x, int y, int z){
    if (x < -HALF_WORLD_SIZE || y < 0 || z < -HALF_WORLD_SIZE || x > HALF_WORLD_SIZE || z > HALF_WORLD_SIZE || y > Y_MAX){
        printf("Provi ad accedere ad un blocco fuori dal mondo\n");
        printf("x: %d y: %d z: %d\n", x, y, z);
        return -1;
    } else{
        int indice = (x + HALF_WORLD_SIZE) * (WORLD_SIZE) * (Y_MAX) + (z + HALF_WORLD_SIZE) * (Y_MAX) + y;
...
int getChunk(int x, int z){
    if (x < -HALF_CHUNK_NUMBER || z < -HALF_CHUNK_NUMBER || x > HALF_CHUNK_NUMBER || z > HALF_CHUNK_NUMBER){
        printf("Provi ad accedere ad un chunk fuori dal mondo\n");
        printf("x: %d z: %d\n", x, z);
        return -1;
    } else {
        return (x + HALF_CHUNK_NUMBER) * (CHUNK_NUMBER) + (z + HALF_CHUNK_NUMBER);
    }
}
...
void initPlayer(GameState *state){
    ...
    initializeCameraScalar(player->camera, initialPos[0], initialPos[1], initialPos[2]);
...
void generateWorld(GameState *state, int mondoX, int mondoZ){
    ...
    //printf("xInizio %d xFine %d zInizio %d zFine %d\n", xInizio, xFine, zInizio, zFine);
    int percentualeCompletamento = 0;
    for (int x = xInizio; x <= xFine; x++){
        if (x % (HALF_WORLD_SIZE / 5) == 0){
            if (percentualeCompletamento != 0){
                printf("%d%% generazione mondo\n", percentualeCompletamento);
            }
            percentualeCompletamento += 10;
        }
        for (int z = zInizio; z <= zFine; z++){
            world->occupati[getIndiceMondo(x, z)][getOccupati(x, z)] = (unsigned char *)malloc(sizeof(unsigned char));
            world->occupati[getIndiceMondo(x, z)][getOccupati(x, z)][0] = 0;
            //int alt = interpolateAtlezze(altezze, x, z);
            setPerlinSeed(3);
            float h = getMax(350 * getPerlin2d(x, z, 0.01, 1) - 200,
            (50 * getPerlin2d(x, z, 0.01, 1) + 50 * getPerlin2d(x, z, 0.02, 2) + 10 * getPerlin2d(x, z, 0.04, 3) + 5 * getPerlin2d(x, z, 0.08, 4) /
            (50 + 50 + 10 + 5) * 100));
            //float h = getMax(200 * getPerlin2d(x, z, 0.05, 1) - 60, ALTITUDE_STANDARD);
            //float h = ALTITUDE_STANDARD;
            //printf("x %d z %d h %f\n", x, z, h);
            for (int y = 0; y <= h; y++){
        ...
        for (int z = zInizio; z <= zFine; z++){
            ...
            while (tex == TEX_LEAVES || tex == TEX_WOOD){
                ...
                if (checkEsisteBlocco(world, x, y, z)){
                    tex = world->cubes[getIndiceMondo(x, z)][getCube(x, y, z)]->tex;
                } else{
                    continue;
                }
            }
            if (y > ALT_SNOW){
                ...
                if (randomSnow > 5){
                    world->cubes[getIndiceMondo(x, z)][getCube(x, y, z)]->tex = TEX_SNOW;
                }
            } else{
                world->cubes[getIndiceMondo(x, z)][getCube(x, y, z)]->tex = TEX_GRASS;
            }
            y++;
    ...
    for (int x = xInizio; x <= xFine; x++){
        for (int z = zInizio; z <= zFine; z++){
            for (int y = ALT_WATER; y >= 0; y--){
                ...
                } else if (!checkEsisteBlocco(world, x, y + 1, z) || world->cubes[getIndiceMondo(x, z)][getCube(x, y + 1, z)]->tex == TEX_WATER){
                    world->cubes[getIndiceMondo(x, z)][getCube(x, y, z)]->tex = TEX_SAND;
                }
            }
        }
    }

    int yPlayer = world->occupati[getIndiceMondo(0, 0)][getOccupati(0, 0)][world->occupati[getIndiceMondo(0, 0)][getOccupati(0, 0)][0]] + 1;
    int y = 0;
    /*for (y = 0; y < 140; y++){
...
void initWorld(GameState *state){
    ...
    world->cubes = (Cube ***)malloc(sizeof(Cube **) * MONDI_MASSIMI * MONDI_MASSIMI);
    world->cubes[getIndiceMondo(0, 0)] = (Cube **)malloc(sizeof(Cube *) * WORLD_SIZE * WORLD_SIZE * Y_MAX);
    world->occupati = (unsigned char ***)malloc(sizeof(unsigned char **) * MONDI_MASSIMI * MONDI_MASSIMI);
    world->occupati[getIndiceMondo(0, 0)] = (unsigned char **)malloc(sizeof(unsigned char *) * WORLD_SIZE * WORLD_SIZE);
...
void initChunks(GameState *state){
    ...
    state->chunks = (Chunk **)malloc(sizeof(Chunk *) * CHUNK_NUMBER * CHUNK_NUMBER);
...
void initZombie(GameState *state){
    ...
    zombie->x = 0;
    zombie->z = -3;
    zombie->y = world->occupati[getIndiceMondo(zombie->x, zombie->z)][getOccupati(zombie->x, zombie->z)][world->occupati[getIndiceMondo(zombie->x, zombie->z)][getOccupati(zombie->x, zombie->z)][0]] + 1;
    state->slime = malloc(sizeof(Slime));
    /*Slime *slime = state->slime;
    slime->x = 0;
    slime->z = -2;
    slime->y = world->occupati[getOccupati(slime->x, slime->z)][world->occupati[getOccupati(slime->x, slime->z)][0]] + 1;*/
    zombie->dir[0] = -1;
    zombie->dir[1] = 1;
}
...
void updateBloccoDaRompere(GameState *state){
    ...
    if (player->timeRotturaBlocco >= TIME_ROTTURA_BLOCCO){
        ...
        int *indiciChunk = getIndiceChunkDiPosizione(state->chunks, player->bloccoDaRompere[0], player->bloccoDaRompere[2]);
        ...
        //setChunk(state, state->chunks[getChunk(indiciChunk[0], indiciChunk[1])]);
        player->bloccoDaRompere[0] = -1;
        player->bloccoDaRompere[1] = -1;
        player->bloccoDaRompere[2] = -1;
        player->timeRotturaBlocco = 0;
...
void updateJump(GameState *state){
    ...
    if (!player->inWater){
        player->camera->position[1] = player->y0 + player->jumpSpeed * player->timeJump - GRAVITY * player->timeJump * player->timeJump;
    } else{
        if (state->spazioPremuto){
            player->camera->position[1] += player->camera->movementSpeed * state->deltaTime;
        } else{
            player->camera->position[1] -= player->camera->movementSpeed * state->deltaTime;
        }
    }
    ...
    for (int z = world->zUpdateInizio; z <= world->zUpdateFine && !fine; z++){
        for (int x = world->xUpdateInizio; x <= world->xUpdateFine && !fine; x++){
            for (int y = world->yUpdateInizio; y <= world->yUpdateFine && !fine; y++){
                if (!checkEsisteBlocco(world, x, y, z) || world->cubes[getIndiceMondo(x, z)][getCube(x, y, z)]->tex == TEX_WATER){
                    continue;
                }
                int posizioneCubo[3] = {x, y, z};
                if (prevY > player->camera->position[1]){
                    if (checkSbatteSotto(player->camera->position, posizioneCubo)){
                        endJump(player, posizioneCubo[1] + 2);
                        fine = true;
                        break;
                    }
                } else if (prevY < player->camera->position[1]){
                    if (checkSbatteSopra(player->camera->position, posizioneCubo)){
...
void updateCheckSopraCubo(GameState *state){
    World *world = state->world;
    Player *player = state->player;
    vec3 posNuova = {player->nuovaPosizione[0], player->camera->position[1], player->nuovaPosizione[1]};
    bool onCubo = false;
    if (sopraMob(posNuova, state->zombie)){
        onCubo = true;
    }
    for (int x = world->xUpdateInizio; x <= world->xUpdateFine && !onCubo; x++){
        for (int z = world->zUpdateInizio; z <= world->zUpdateFine && !onCubo; z++){
            for (int y = world->yUpdateInizio; y <= world->yUpdateFine && !onCubo; y++){
                if (!checkEsisteBlocco(world, x, y, z) || world->cubes[getIndiceMondo(x, z)][getCube(x, y, z)]->tex == TEX_WATER){
                    continue;
                }
                int posizioneCubo[3] = {x, y, z};
...
void updateCollisionPlayer(GameState *state){
    ...
    for (int z = world->zUpdateInizio; z <= world->zUpdateFine && (!colX || !colZ); z++){
        for (int x = world->xUpdateInizio; x <= world->xUpdateFine && (!colX || !colZ); x++){
            for (int y = world->yUpdateInizio; y <= world->yUpdateFine && (!colX || !colZ); y++){
                if (!checkEsisteBlocco(world, x, y, z) || world->cubes[getIndiceMondo(x, z)][getCube(x, y, z)]->tex == TEX_WATER){
                    continue;
                }
                int posizioneCubo[3] = {x, y, z};
                if (collisionPlayerY(player->camera->position[1], posizioneCubo[1])){
                    if (collisionPlayerXZ(posConX, posizioneCubo[0], posizioneCubo[2])){
                        colX = true;
                    }
                    if (collisionPlayerXZ(posConZ, posizioneCubo[0], posizioneCubo[2])){
                        colZ = true;
                    }
                }
            }
...
void updateCreative(GameState *state){
    if (state->spazioPremuto){
        state->player->camera->position[1] += (state->player->camera->movementSpeed / 2.0f) * state->deltaTime;
...
void updateGameState(GameState *state){
    ...
    if (xInt < player->previusPosition[0] - CHUNK_SIZE || xInt > player->previusPosition[0] + CHUNK_SIZE ||
        zInt < player->previusPosition[1] - CHUNK_SIZE || zInt > player->previusPosition[1] + CHUNK_SIZE){
        setDrawStruct(state);
        player->previusPosition[0] = xInt;
        player->previusPosition[1] = zInt;
    }
    if (!player->inWater && checkEsisteBlocco(world, xInt, yInt, zInt) && world->cubes[getIndiceMondo(xInt, zInt)][getCube(xInt, yInt, zInt)]->tex == TEX_WATER){
        player->inWater = true;
