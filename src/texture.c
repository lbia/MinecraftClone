...
Texture *createMobTexture(){
    Texture *mobTextures = malloc(sizeof(Texture) * NUMERO_MOB_TEXTURES);
    initializeTexture(&(mobTextures[TEX_ZOMBIE]), "res/mob/zombie.png");
}
...
BlockTexture *createBlockTexture(){
    ...
    initializeTexture(&(blockTextures[TEX_LEAVES].bottom), "res/blocks/stone.png");
